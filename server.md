...
Session keys will last for 1 week. After 1 week, their cookie will expire and they will have to log in again.
...
For the login routes, the error respones will not be included as there are too many, to see the routes in full detail refer to the flowchart at the end of this section.
...
- **Method:** `POST`
- **Description:** Resend verification email sent in previous route. This route is triggered by the frontend which is why it has a recaptchaToken.
...
- **Method:** `POST`
- **Description:** This route is accessed through the link sent to their old email address in the previous route. This route with set the user's session to invalid in the database (to invalidate any further uses of that session) and the user will be sent an email that follows the forgot password flow which will validate their session in the database. The user is also assigned a random password. These together make it so that the account is locked down until the user changes their password by following the link in the email.
