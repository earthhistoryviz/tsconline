...
export const Settings = observer(function Settings() {
  ...
  return (
    <div className="settings-container" style={{ background: theme.palette.backgroundColor.main }}>
      ...
      <TSCPopupDialog open={state.settingsTabs.selected != "datapacks" && state.datapackSelection.isDirty} title="Confirm Datapack Selection Change" message="You have unsaved change on datapack selction. Do you want to save the change?" onYes={async () => {
      ...
      }} onNo={() => { actions.setSelectedDatapacks([]); actions.setUnselectedDatapacks([]); actions.setIsDirty(false); }} onClose={() => { actions.setSelectedDatapacks([]); actions.setUnselectedDatapacks([]); actions.setIsDirty(false); }}></TSCPopupDialog>
    </div>
  );
});
