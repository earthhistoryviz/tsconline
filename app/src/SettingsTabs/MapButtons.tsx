    ...
    <svg className="transect-lines">
      ...
      <g filter="url(#dropshadow)">
        {Object.entries(transects).map(([name, transect]) => {
          ...
          let startPosition = getPositionOfPointBasedOnBounds(bounds, start, frameWidth, frameHeight);
          if (!startPosition) throw new Error(`MapInfo bounds are neither vertical or rectangular for ${bounds}`);
          let endPosition = getPositionOfPointBasedOnBounds(bounds, end, frameWidth, frameHeight);
...
function getFaciesIcon(
  ...
  let rockType = getRockTypeForAge(column, currentFaciesOptions.faciesAge, setSelectedMapAgeRange);
  return (
