...
export const Results = observer(
  ({ term, scrollRef }: { term: string; scrollRef: React.RefObject<TableVirtuosoHandle> }) => {
    const { state, actions } = useContext(context);
    ...
    function Row(props: { row: temp; index: number }) {
      ...
      function handleClick() {
        toggleItemState(row.key);
        actions.setGeneralSearchIndex(index);
      }
      return (
        <React.Fragment>
          <TableRow sx={{ "& > *": { borderBottom: "unset" }, width: "50vw" }}>
            ...
            <TableCell
              style={{ width: "100vw" }}
              onClick={() => toggleItemState(row.key)}
              component="th"
              scope="row"
              align="center">
              {row.key}
    ...
    const VirtuosoTableComponents: TableComponents<temp> = {
      ...
      TableRow: ({ item: _item, ...props }) => <TableRow {...props} style={{ width: "60vw" }} />,
      TableBody: React.forwardRef<HTMLTableSectionElement>((props, ref) => (
        <TableBody {...props} ref={ref} style={{ width: "60vw" }} />
      ))
    };
    useEffect(() => {
      scrollRef.current?.scrollToIndex(state.settingsTabs.generalSearchIndex);
    });
    return (
      <Paper style={{ height: "40vw", width: "60vw", marginTop: "2vh" }}>
        <TableVirtuoso
          ref={scrollRef}
          data={arr}
