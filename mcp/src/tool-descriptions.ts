...
- Do not ask the user to log in just to use this tool with public or otherwise accessible datapacks.
  ...
  - Zone label orientation: { "Period": { "orientation": "vertical" } } or { "orientation": "normal" } for horizontal. If the named column is a parent folder (not itself a Zone), orientation is applied to Zone descendants under it.
  ...
  Input:
  - { datapackTitles: string[]; overrides?: { fonts?: Record<FontTarget, FontSettings>; [key: string]: unknown }; columnToggles?: Record<string, { on?: boolean; width?: number; enableTitle?: boolean; showAgeLabels?: boolean; orientation?: "normal" | "vertical"; fonts?: Record<FontTarget, FontSettings>; }>; useCache?: boolean; isCrossPlot?: boolean; sessionId?: string }
  Output data:
  ...
  - columnToggles entries may include orientation ("normal" = horizontal labels, "vertical" = vertical labels). Applies to Zone columns; if the key is a parent folder, orientation is applied to Zone descendants under it.
